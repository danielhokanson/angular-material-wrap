  ...
  template: `
    <div class="upload-container">
      ...
      <amw-file-input
        ...
        placeholderText="Drop files here or click to browse"
...
export class DocumentUploadComponent {
  ...
  onValidation(result: FileValidationResult) {
    if (!result.valid) {
...
### Pattern 6: Advanced Form Controls
...
@Component({
  ...
  template: `
    <form class="event-form">
      ...
      <!-- Color Picker for Event Category -->
      <amw-color-picker
        [value]="eventColor"
        [mode]="'palette'"
        [showPreview]="true"
      ...
      <!-- Range Slider for Attendee Count -->
      <label>Expected Attendees: {{ attendeeRange.start }} - {{ attendeeRange.end }}</label>
      <amw-range-slider
        [min]="0"
        [max]="500"
        [step]="10"
        [(rangeValue)]="attendeeRange"
        [showTicks]="true"
        [showLabels]="true"
        (rangeChange)="onAttendeeRangeChange($event)">
      </amw-range-slider>
...
export class EventFormComponent {
  eventTime = '14:00';
  eventColor = '#FF5722';
  attendeeRange = { start: 50, end: 100 };
  eventDescription = '';
  ...
  onAttendeeRangeChange(range: { start: number; end: number }) {
    console.log('Attendee range:', range);
  }
}
```
